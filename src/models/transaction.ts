...
const getTransactionId = (transaction: TransactionWithoutId): string => {
  const txInContent: string = transaction.txIns.map(txIn => `${txIn.txOutId}${txIn.txOutIndex}`).join('')
  const txOutContent: string = transaction.txOuts.map(txOut => `${txOut.address}${txOut.amount}`).join('')
...
const isValidTransactionsStructure = (transactions: Transaction[]) => {
  ...
  return transactions.map((t) => isValidTransactionStructure(t)).reduce((a,b) => a && b, true)
}

const validateTxIn = (txIn: TxIn, transaction: Transaction, unspentTxOuts: UnspentTxOut[]): boolean => {
  const referencedUTxOut: UnspentTxOut | undefined = unspentTxOuts.find((uTxO) => uTxO.txOutId === txIn.txOutId && uTxO.txOutId === txIn.txOutId);
  if (!referencedUTxOut) {
...
const validateTxIns = (txIns: TxIn[], transaction: Transaction, unspentTxOuts: UnspentTxOut[]): boolean => {
  return txIns.map(txIn => validateTxIn(txIn,transaction,unspentTxOuts)).reduce((a,b) => a && b, true)
}

const validateTxOut = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]) => {
  const totalTxInValue = transaction.txIns.map(txIn => getTxInAmount(txIn, aUnspentTxOuts)).reduce((a,b) => a+b)
  const totalTxOutValue = transaction.txOuts.map(txOut => txOut.amount).reduce((a,b) => a+b)
...
const validateBlockTransactions = (transactions: Transaction[], unspentTxOuts: UnspentTxOut[], blockIndex: number) => {
  ...
  const isValidTransaction = otherTransactions.map(tx => validateTransaction(tx, unspentTransactionOut)).reduce(andReducer, true)
...
const updateUnspentTxOuts = (newTransactions: Transaction[]) => {
  const newUnspentTxOuts: UnspentTxOut[] = newTransactions
      .flatMap((t) => {
          return t.txOuts.map((txOut, index): UnspentTxOut => ({ txOutId: t.id, txOutIndex: index, ...txOut  }));
      });

  const consumedTxOuts: UnspentTxOut[] = newTransactions
      .flatMap((t) => t.txIns)
      .map((txIn) => ({ txOutId: txIn.txOutId,txOutIndex: txIn.txOutIndex, address: '', amount: 0}));

  const resultingUnspentTxOuts = [...unspentTransactionOut
      .filter(((uTxO) => !findUnspentTxOut(uTxO.txOutId, uTxO.txOutIndex, consumedTxOuts))), ...newUnspentTxOuts];

  unspentTransactionOut = resultingUnspentTxOuts
};

export const processTransactions = (transactions: Transaction[], blockIndex: number) => {
  if(!isValidTransactionsStructure(transactions)){
    return
  }

  if(!validateBlockTransactions(transactions,unspentTransactionOut, blockIndex)){
    return
  }
