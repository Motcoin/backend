...
const getAdjustedDifficulty = (latestBlock: Block, chain: Block[]) => {
  const previousAdjustmentBlock: Block =
    chain[chain.length - DIFFICULTY_ADJUSTMENT_INTERVAL];
  const timeExpected: number =
    BLOCK_GENERATION_INTERVAL * DIFFICULTY_ADJUSTMENT_INTERVAL;
  const timeTaken: number =
    (latestBlock.timestamp - previousAdjustmentBlock.timestamp) / 1000;
...
export const getDifficulty = (chain: Block[]) => {
  ...
  return latestBlock.index % DIFFICULTY_ADJUSTMENT_INTERVAL === 0 &&
    latestBlock.index !== 0
    ? getAdjustedDifficulty(latestBlock, chain)
...
export const mine = (block: Block) =>
  pool.exec("mine", [block]).then((response: any) => {
